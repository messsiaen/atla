...
from utilities.replay_buffer import TransReplayBuffer, TransReplayBufferPriority, EpisodeReplayBuffer
...
class PGTrainer(object):
    def __init__(self, args, model, env, logger, constraint_model=None):
        ...
        if self.args.replay:
            if not self.episodic:
                if self.args.priority_buffer:
                    self.replay_buffer = TransReplayBufferPriority(
    ...
    def get_loss(self, batch, no_sum=False):
    ...
    def policy_replay_process(self, stat):
        if self.args.priority_buffer: 
            batch, _, _ = self.replay_buffer.get_batch(self.args.batch_size)
        else:
    ...
    def value_replay_process(self, stat):
        if not self.args.priority_buffer:
            batch = self.replay_buffer.get_batch(self.args.batch_size)
            batch = self.behaviour_net.Transition(*zip(*batch))
            indices, weights = [], []
            self.value_transition_process(stat, batch, indices, weights)
        else:
            batch_data, indices, weights = self.replay_buffer.get_batch(self.args.batch_size)
            batch = self.behaviour_net.Transition(*zip(*batch_data))
            self.value_transition_process(stat, batch, indices, weights)
    ...
    def policy_transition_process(self, stat, trans):
        if self.args.continuous:
            policy_loss, _, logits = self.get_loss(trans, no_sum=False)
    ...
    def value_transition_process(self, stat, trans, indices, weights):
        if self.args.priority_buffer:
            _, value_loss, _ = self.get_loss(trans, no_sum=True)
            self.replay_buffer.update_priorities(indices, value_loss)
            if not th.is_tensor(weights):
                weights = th.from_numpy(weights).float().to(value_loss.device)
            else:
                weights = weights.to(value_loss.device)
    ...
    def run(self, stat, episode, hyperparams = None):
        self.behaviour_net.train_process(stat, episode, self, hyperparams)
